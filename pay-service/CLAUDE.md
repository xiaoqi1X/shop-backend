...
> 支付微服务，端口 8086，数据库 `hm-pay`。负责支付单生成、余额支付、支付状态管理。通过 OpenFeign 远程调用 `user-service` 扣减余额、`trade-service` 标记订单支付成功。
...
4. **通知订单服务**：远程调用 `TradeClient.markOrderPaySuccess(bizOrderNo)`
   - `trade-service` 将订单状态更新为已付款
...
| `PayOrderServiceImpl` | `trade-service` | `TradeClient.markOrderPaySuccess` | 标记订单支付成功 |
...
|------|------|
| `hm-common` | 公共模块 |
| `hm-api` | Feign API 模块（UserClient、TradeClient） |
