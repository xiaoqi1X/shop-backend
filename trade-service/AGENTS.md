...
> 交易/订单微服务，端口 8085，数据库 `hm-trade`。负责订单创建、订单查询、支付成功后订单状态更新。创建订单时通过 OpenFeign 远程调用 `item-service` 查询商品和扣减库存，调用 `cart-service` 清理购物车；支付成功状态由 RocketMQ 消息触发更新。
...
└── src/main/
    ...
    │       ├── mq/
    │       │   └── PaySuccessMessageConsumer.java # RocketMQ 支付成功消息消费者
    │       └── service/impl/
...
| `markOrderPaySuccess` | PUT | `/orders/{orderId}` | `orderId` 路径参数 | 标记订单支付成功（也被 MQ 消费者复用） |
...
### 3.2 支付成功消息消费（`PaySuccessMessageConsumer`）

`trade-service` 订阅 `trade_pay_success_topic` 的 `pay_success` 标签消息。

**执行流程**：

1. RocketMQ 推送支付成功消息，消息体为业务订单 ID 字符串。
2. 消费者将消息体按 UTF-8 解码并转换为 `Long orderId`。
3. 调用 `IOrderService.markOrderPaySuccess(orderId)`。
4. 更新成功返回 `CONSUME_SUCCESS`；解析或业务异常返回 `RECONSUME_LATER`，等待 RocketMQ 重试。

### 3.3 标记订单支付成功（`OrderServiceImpl.markOrderPaySuccess`）

根据订单 ID 更新订单主表：

- `status = 2`，表示已付款未发货。
- `payTime = LocalDateTime.now()`，记录支付时间。
...
`PaySuccessMessageConsumer` 是 Spring 管理的 RocketMQ 消费者：

- Bean 初始化时创建 `DefaultMQPushConsumer` 并订阅支付成功消息。
- 收到消息后解析订单 ID，并调用 `orderService.markOrderPaySuccess(orderId)`。
- Bean 销毁时关闭 consumer。
...
| Consumer Group | `trade-service-consumer-group` |
